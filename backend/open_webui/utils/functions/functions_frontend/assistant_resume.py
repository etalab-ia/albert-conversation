...
class Pipe:
    ...
    async def pipe(
        ...
        if state_dict == None:
            ...
            print("STATE DOES NOT EXISTS : Initialised it")
        else:
            state_dict = ast.literal_eval(state_dict)
            # print(f"STATE EXISTS {state_dict["step"]}")
...
""",
        ...
        async def demande_sommaire_ou_gen(synth_id, state_dict):
            ...
            print("REPONSE SOMMAIRE:", response)
...
""",
        ...
        async def genere_et_valide_sommaire(synth_id, state_dict):
            ...
            print(f"Instructions : {state_dict['instructions_supp']}")
        ...
        async def genere_resume_et_valide(synth_id, state_dict, session):
            ...
            """
            file_content = state_dict["file_content"]
        ...
        async def relance_synthese_si_modif(synth_id, state_dict):
            ...
            print("COLLECTION : ", state_dict["collection_id"])
            data = {
        ...
        try:
            if state_dict["step"] == 0:
                ...
                if not file_content:  # doc non fourni, déjà géré
                    return body
                state_dict["step"] = 1
                ...
                print("LA 3")
                # Le step 1 est consommé dans la même session grâce à event_call
                # donc on continue...

            if state_dict["step"] == 3:  # Générer un sommaire à faire valider
                print("LA 4")
            ...
            if state_dict["step"] == 31:
                ...
                if user_confirm in ["ok", "oui", "yes", "valider", ""]:
                    ...
                    # relance génération sommaire
                    return await genere_et_valide_sommaire(synth_id, state_dict)

            if (
                state_dict["step"] == 4
            ):  # Génère le résumé, puis le proposer à validation
                return await genere_resume_et_valide(synth_id, state_dict, session)

            if state_dict["step"] == 41:  # Validation finale du résumé/ajout de modif
                user_confirm = last_user_message.strip().lower()
                if user_confirm in ["ok", "oui", "yes", "valider", ""]:
...
---
            ...
            if state_dict["step"] == 99:
                ...
                try:
                    ...
                    print(f"No collections to delete ({response.tex})")
                if state_dict["response_sommaire"] == False:
