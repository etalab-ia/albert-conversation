...
class AsyncHelper:
    ...
    async def perform_brave_search_async(client, query, k=2, num_queries=2):
        ...
        try:
            #async with requests.get(url, headers=headers, params=params) as resp:
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=60)) as session:
            ...
            if num_queries > 1:
                final_report, final_prompt = await AsyncHelper.generate_final_report_async(
                    client, token_counter, user_query, aggregated_contexts, '', prompt_suffix, max_tokens, lang, event_emitter
                )
            else:
