...
async def search_api_albert(prompt: str, k: int=5, collection_ids: list=None)-> list:
    """
    ...
    """
    print("COLLECTIONS WANTED : ", collection_ids)
    print(OPENAI_API_BASE_URL, OPENAI_API_KEYS)
    docs = []
    names = []
    for coll in collection_ids:
        data = {"collections": [coll], "k": k, "prompt": prompt}
        response = requests.post(url="https://albert.api.etalab.gouv.fr/v1/search", json=data, headers={"Authorization": f"Bearer {OPENAI_API_KEYS}"})
        docs_coll = []
        #print(response.text)
...
class AsyncHelper:
    ...
    async def call_openrouter_async(client, token_counter, messages, max_tokens=2048, step=False):
        ...
        print(os.getenv('REDACTOR_MODEL'), os.getenv('ANALYTICS_MODEL'), os.getenv('DEFAULT_MODEL'))
        model = {
    ...
    @staticmethod
    async def perform_brave_search_async(query, k=2, num_queries=2):
    ...
    async def is_page_useful_async(client, token_counter, user_query, page_text, lang='fr'):
        ...
        print("USEFUL MESSAGES : ", messages)
    ...
    @staticmethod
    async def extract_relevant_context_async(client, token_counter, user_query, search_query, page_text, max_tokens=1024, lang='fr'):
        if not page_text:
            return ""
...
#        return messages[-1]['content'], prompt
    @staticmethod
    async def generate_final_prompt(client, token_counter, user_query, all_contexts, urls, prompt_suffix, max_tokens, lang='fr', event_emitter=None):
        if not all_contexts:
            return "No relevant information found to answer your query.", ""
    ...
    async def process_link(client, token_counter, link, user_query, search_query, log, lang='fr', num_queries=2, event_emitter=None):
        ...
        if usefulness == "Oui" and num_queries > 1:
            context = await AsyncHelper.extract_relevant_context_async(client, token_counter, user_query, search_query, page_text, lang=lang)
            if context:
    ...
    @staticmethod
    async def process_query_api(client, token_counter, user_query, search, k, log, lang='fr', collections_ids=None):
        log.append(f"Fetching content from: {user_query}")
        try:
            #docs = await search_api(search, k, collection)
            docs = await search_api_albert(search, k, collections_ids)
            ...
            usefulness_tasks = [AsyncHelper.is_page_useful_async(client, token_counter, user_query, doc, lang) for doc in docs]
    ...
    @staticmethod
    async def process_useful_api_docs(client, token_counter, user_query, useful_docs, log, lang='fr'):
        if not useful_docs:
            return []
            
        context_tasks = [AsyncHelper.extract_relevant_context_async(client, token_counter, user_query, user_query, doc, max_tokens=512, lang=lang) for doc in useful_docs]
...
async def async_research(client, user_query, internet, iteration_limit, prompt_suffix=None, max_tokens=1024, num_queries=2, k=5, lang='fr', collections_ids=None, event_emitter=None):
    ...
    print("STARTING RESEARCH...DADADA")
    ...
    try:
        ...
        while iteration < iteration_limit:
            ...
            if internet:
                ...
                for query in new_search_queries[:num_queries]:
                    search_results.append(await AsyncHelper.perform_brave_search_async(query, k, num_queries))
                ...
                search_api_tasks = [AsyncHelper.process_query_api(client, token_counter, user_query, search, k, log_messages, lang, collections_ids) for search in new_search_queries[:num_queries]]
            ...
            print("NEW SEARCH QUERIES:", new_search_queries)
        ...
        if num_queries > 1:
            final_prompt = await AsyncHelper.generate_final_prompt(
                client, token_counter, user_query, aggregated_contexts, '', prompt_suffix, max_tokens, lang
            )
            print("FINAL PROMPT : ", final_prompt)
        else:
            final_prompt = str(aggregated_contexts) + "\n Voilà les documents que j'ai trouvé pour ta recherche, utilises ces informations pour répondre à la question de l'utilisateur en format markdown. Donnes les sources et liens intéressants si il y en a.", ""
        ...
        return final_prompt, "\n".join(log_messages), total_input_tokens, total_output_tokens, aggregated_chunks, elapsed_time
...
def run_research(client, user_query, internet, iteration_limit=10, prompt_suffix='', max_tokens=2048, num_queries=2, k=5, lang='fr', collections_ids=None, event_emitter=None):
    ...
    def _run():
        ...
        try:
            ...
            result_container['result'] = loop.run_until_complete(
                async_research(client, user_query, internet, iteration_limit, prompt_suffix, max_tokens, num_queries, k, lang, collections_ids, event_emitter)
            )
        finally:
            loop.close()
